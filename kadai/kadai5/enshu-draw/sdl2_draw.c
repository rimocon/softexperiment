...
int main(int argc, char *argv[])
{
    ...
    // 線を描画
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0); // 生成したRCに描画色として緑を設定
    ...
    // 四角形（塗りつぶしあり）を描画
    SDL_SetRenderDrawColor(renderer, 255, 255, 0, 127); // 生成したRCに描画色として黄を設定
    ...
    // 文字列を描画
    stringColor(renderer, 0, 0, "FUCKIN CRAZY MAN", 0xffffffff); // 白で描画
