...
int main( int argc,char *argv[]){
	...
	SDL_Window *window = SDL_CreateWindow("kadai2",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,600,500,0); //ウィンドウの生成
	SDL_Color white = {0xFF,0xFF,0xFF}; //白
	SDL_Color black = {0x00,0x00,0x00}; //黒
	if (window == NULL){
	...
	SDL_Rect dst_rect2 = {210,10,100,50};
	SDL_Rect dst_rect3 = {410,10,100,50};
	...
	SDL_Rect dst_like = {10,100,510,400};
	...
	SDL_Rect dst_button2 = {210,10,100,50};
	...
	SDL_Rect dst_button3 = {410,10,100,50};
			...
			switch(event.type){
				case SDL_KEYDOWN:
					if (event.key.keysym.sym == SDLK_RIGHT ){ //右矢印を押された時
						...
						if( position == 1){
							rectangleColor(renderer,210,10,310,60,0xff000000);
						}
						else if (position == 2){
							rectangleColor(renderer,410,10,510,60,0xff000000);
						}
						SDL_RenderPresent(renderer);
					}
						...
						if( position == 0){
							...
							else if(color ==5) SDL_SetRenderDrawColor(renderer,255,255,0,0);
							else if(color ==6) SDL_SetRenderDrawColor(renderer,0,255,255,0);
